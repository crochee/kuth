...
pub struct NilString(String);

impl FromRedisValue for NilString {
    fn from_redis_value(v: &Value) -> RedisResult<Self> {
        match *v {
            Value::Data(ref bytes) => Ok(Self(from_utf8(bytes)?.to_string())),
            Value::Okay => Ok(Self("OK".to_string())),
            Value::Status(ref val) => Ok(Self(val.to_string())),
            Value::Nil => Ok(Self("".to_string())),
